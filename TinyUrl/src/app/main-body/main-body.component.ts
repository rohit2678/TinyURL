  ...
  doConvert() {
    ...
    if (!this.longURL) {
      ...
      if (dataFound) {
        ...
        const URL = URLArray.find(
          (URL: { user: string; longURL: string; shortURL: string }) =>
            URL.longURL == this.longURL
        );
        if (URL) {
          alert('Short URL Already created...');
          this.shortURL = URL.shortURL;
          if (this.loginService.loggedUserName != URL.user) {
            this.loginService.URLData.push({
              user: this.loginService.loggedUserName,
              longURL: this.longURL,
              shortURL: this.shortURL
            });
            localStorage.setItem(
              'URLData',
              JSON.stringify(this.loginService.URLData)
            );
          }
        } else {
